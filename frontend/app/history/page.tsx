    ...
    <Link href={`/place/${place.id}`} className="block">
      <div className="bg-gray-900 rounded-lg overflow-hidden cursor-pointer hover:bg-gray-800 transition-colors">
        <div className="relative">
          ...
          <div className="absolute top-2 left-2 bg-gradient-to-r from-green-400 to-green-500 text-white text-xs px-2 py-1 rounded">
            방문기록
          </div>
        </div>
        <div className="p-3">
          ...
          <div className="flex items-center justify-between mt-3 pt-2 border-t border-gray-700">
            ...
            <button className="text-xs text-green-400 hover:text-green-300 transition-colors">재시청</button>
          </div>
        </div>
      </div>
    </Link>
...
export default function HistoryPage() {
  ...
  // 방문한 맛집만 필터링 (실제로는 방문 기록이 있는 항목들)
  const userHistoryPlaces = sampleHistoryPlaces.filter(place => visited.includes(place.id))

  // 필터링된 데이터 생성
  const filteredPlaces = useMemo(() => {
    let filtered = userHistoryPlaces
    ...
    return filtered
  }, [filters, searchQuery, sortBy, userHistoryPlaces])
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-medium">{userHistoryPlaces.length}개</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-300">이번 주</span>
                  <span className="text-white">{groupedPlaces.thisWeek.length}개</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-300">오늘</span>
                  <span className="text-white">{groupedPlaces.today.length}개</span>
                </div>
              </div>
            </div>
        ...
        <section className="flex-1 p-6">
          ...
          <div className="xl:hidden mb-4">
            <button 
              ...
              className="flex items-center gap-2 bg-gradient-to-r from-green-400 to-green-500 text-white px-4 py-2 rounded-lg hover:from-green-500 hover:to-green-600 transition-all duration-200 shadow-md"
            >
            ...
            <div className="bg-black w-full lg:w-96 h-5/6 lg:h-auto max-h-screen overflow-y-auto rounded-t-lg lg:rounded-lg">
              ...
              <div className="p-4 border-t border-gray-800 flex gap-3">
                ...
                <button 
                  ...
                  className="flex-1 bg-gradient-to-r from-green-400 to-green-500 text-white py-2 rounded-lg hover:from-green-500 hover:to-green-600 transition-all"
                >
                  필터 적용
                </button>
              </div>
